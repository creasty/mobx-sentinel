...
- `Watcher` detects changes in models automatically.
  ...
  - `@watch` annotation can be used for `@computed` getters, and for fields holding an `observable.box()`, `.array()`, `.map()` or `.set()` instead of being annotated with `@observable`, e.g., `@watch readonly count = observable.box(0)`.
